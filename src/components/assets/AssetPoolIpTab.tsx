    ...
    <div className="flex flex-col gap-4 animate-in fade-in duration-500">
      <div className="flex items-center justify-between pb-2">
        <div className="flex flex-col text-sm">
          <h3 className="font-bold text-white text-lg">IP 视角</h3>
        </div>
      </div>
      ...
      <div className="flex gap-4 items-center">
        <Input
          ...
          classNames={{ inputWrapper: 'bg-white/5 border border-white/10 h-10 w-72' }}
      ...
      <div className="border border-white/10 rounded-2xl bg-white/[0.01]">
        <Table removeWrapper aria-label="IP Assets Table" classNames={{ th: 'bg-white/5 text-[10px] uppercase font-black tracking-widest text-apple-text-secondary border-b border-white/10 py-3', td: 'border-b border-white/5 py-4 text-sm' }}>
          <TableHeader>
          ...
          <TableBody emptyContent={<div className="py-20 text-apple-text-tertiary">此资产池下暂无 IP 事实资产。</div>} isLoading={isPending} loadingContent={<Skeleton className="h-40 w-full rounded-b-2xl" />}>
            {items.map((it) => (
              <TableRow key={it.id}>
                <TableCell><span className="font-mono font-bold tracking-tight text-white">{it.display_name}</span></TableCell>
                <TableCell><span className="text-xs text-apple-text-secondary font-mono">{it.normalized_key}</span></TableCell>
        ...
        {total > 0 && (
          <div className="flex justify-between items-center px-4 py-3 border-t border-white/5">
            <span className="text-[10px] uppercase font-black tracking-widest text-apple-text-tertiary">IP 事实资产 {total} 项</span>
